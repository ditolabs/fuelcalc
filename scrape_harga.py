...
HARGA_FALLBACK = [
    ...
    {"nama": "Pertamax Turbo", "harga": 20750, "warna": "#ef4444", "ron": "RON 98", "tipe": "nonsubsidi"},
    {"nama": "Dexlite",        "harga": 23000, "warna": "#3b82f6", "ron": "CN 51",  "tipe": "nonsubsidi"},
    {"nama": "Pertamina Dex",  "harga": 24800, "warna": "#8b5cf6", "ron": "CN 53",  "tipe": "nonsubsidi"},
    {"nama": "Solar",          "harga":  6800, "warna": "#eab308", "ron": "CN 48",  "tipe": "subsidi"},
]

OUTPUT_FILE = Path("harga.json")
TARGET_URL  = "https://mypertamina.id/about/product-price"

# Regex untuk tiap produk — urutan penting (Turbo harus sebelum Pertamax biasa)
PATTERNS = [
    ("Pertalite",      r"pertalite[\s\S]{0,300}?Rp[\s.]*([\d.]+)"),
    ("Pertamax Turbo", r"pertamax\s*turbo[\s\S]{0,300}?Rp[\s.]*([\d.]+)"),
    ("Pertamax",       r"pertamax(?!\s*turbo|green)[\s\S]{0,300}?Rp[\s.]*([\d.]+)"),
    ("Dexlite",        r"dexlite[\s\S]{0,300}?Rp[\s.]*([\d.]+)"),
    ("Pertamina Dex",  r"pertamina\s*dex[\s\S]{0,300}?Rp[\s.]*([\d.]+)"),
    ("Solar",          r"bio\s*solar[\s\S]{0,300}?Rp[\s.]*([\d.]+)"),
]
...
def scrape_requests() -> dict[str, int] | None:
    """Coba scrape dengan requests (cepat, ringan)."""
    try:
        import requests
        headers = {
            "User-Agent": (
        ...
        if len(found) >= 3:
            print(f"[requests] Berhasil: {found}")
            return found
        print(f"[requests] Hanya menemukan {len(found)} harga — coba Playwright")
...
def scrape_playwright() -> dict[str, int] | None:
    """Fallback: scrape via Playwright headless Chromium."""
    try:
        ...
        if len(found) >= 3:
            print(f"[playwright] Berhasil: {found}")
            return found
        print(f"[playwright] Hanya menemukan {len(found)} harga")
    except Exception as e:
